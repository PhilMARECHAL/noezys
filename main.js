/* ═══════════════════════════════════════════════════════════
   NOEZYS — main.js
   V1 · Canvas cosmos + scroll interactions
   ═══════════════════════════════════════════════════════════ */
...
(function () {
  ...
  function createParticle() {
    ...
    return {
      ...
      color: isGold
        ? `rgba(201, 168, 76, VAR_OPACITY)`
        : `rgba(140, 170, 255, VAR_OPACITY)`,
    };
  }
  ...
  function drawCosmos(time) {
    ...
    for (const line of energyLines) {
      ...
      lineGrad.addColorStop(0, `rgba(58, 123, 255, 0)`);
      lineGrad.addColorStop(Math.max(0, line.progress - 0.15), `rgba(58, 123, 255, 0)`);
      lineGrad.addColorStop(line.progress, `rgba(58, 123, 255, ${line.opacity})`);
      lineGrad.addColorStop(Math.min(1, line.progress + 0.05), `rgba(58, 123, 255, 0)`);
    ...
    horizonGrad.addColorStop(0.3, `rgba(201, 168, 76, ${horizonOpacity * 0.6})`);
    horizonGrad.addColorStop(0.5, `rgba(58, 123, 255, ${horizonOpacity})`);
    horizonGrad.addColorStop(0.7, `rgba(201, 168, 76, ${horizonOpacity * 0.6})`);
    horizonGrad.addColorStop(1, 'rgba(201, 168, 76, 0)');
    ...
    const shimmerGrad = ctx.createRadialGradient(W * 0.5, H * 0.48, 0, W * 0.5, H * 0.48, 120);
    shimmerGrad.addColorStop(0, `rgba(201, 168, 76, ${shimmerOpacity})`);
    shimmerGrad.addColorStop(1, 'rgba(201, 168, 76, 0)');
    ctx.fillStyle = shimmerGrad;
    ctx.fillRect(W * 0.5 - 120, H * 0.48 - 120, 240, 240);
  ...
  if (!prefersReducedMotion) {
    ...
    // Static fallback: draw just the radial gradient
    resize();
    const grad = ctx.createRadialGradient(W * 0.5, H * 0.48, 0, W * 0.5, H * 0.48, Math.max(W, H) * 0.7);
    grad.addColorStop(0, '#0C1838');
    grad.addColorStop(0.5, '#071020');
  ...
  // Stagger in hero elements after page load
  ...
  if (!prefersReducedMotion) {
    ...
    // Shift lines — staggered reveal
    const lineObserver = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const lines = entry.target.closest('.shift__lines')
              ? entry.target.closest('.shift__lines').querySelectorAll('.reveal-line')
              : [entry.target];
    ...
    document.querySelectorAll('.shift__lines').forEach((group) => {
