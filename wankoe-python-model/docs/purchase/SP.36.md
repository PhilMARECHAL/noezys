...
- **RE-SPECIFIED 2026-08-16 (benchmark improvement Q4, client option 2): FULL-STREAM dynamic classifier.** The machine ingests the WHOLE 0/1.5 fines stream — **13.6 t/h (mode G) to 23.3 t/h (mode F)** — and cuts at d50c 65 µm. Reference machine class: **Hosokawa Ventoplex C25V** (24 t/h max feed, catalog fines-yield point at df97 63 µm on limestone, INTERNAL recirculation fan) — vendor-candidates dossier, candidate 2, now the best pick.
- **Consequences on the purchase file**: the external **420 m³/h fan (PD-3, 2026-08-15) is CANCELLED** — the air circuit is internal to the machine; the **CL.38 product cyclone is WITHDRAWN** from the purchase list (internal recirculation); the **polishing bag filter REMAINS**, re-scoped as the classifier-circuit vent filter (ePTFE, ≤ 5 mg/Nm³, "may contain RCS pending XRD" — RCS assessment unchanged).
- **HONESTY CEILING (client-accepted)**: the natural sub-65 µm content of the fines is **0.60–0.66 %** (engine PSD, modes G/F) — the full-stream machine harvests **at most ~430 t/y of UltraFin** (vs 284 t/y today). Kilotonne-scale quartz-free output would require a grinding circuit (client decision: not this round). The machine's value = capture ALL the natural ultrafines + full-stream de-dusting of the fines product + readiness for a future mill circuit.
